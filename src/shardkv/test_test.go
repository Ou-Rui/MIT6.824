...
func TestMissChange(t *testing.T) {
	...
	DPrintf("[Tester]: Get Put round 1 OK, config 1\n")
	...
	DPrintf("[Tester]: shutdown server 0 in all group. \n")

	cfg.join(2)
	cfg.leave(1)
	cfg.leave(0)
	DPrintf("[Tester]: Join 2, leave 0+1, config[5] = 2. \n")
	...
	DPrintf("[Tester]: Get Append round 2 OK, config 5 \n")

	cfg.join(1)
	DPrintf("[Tester]: Join 1, config[6] = 1+2. \n")
	...
	DPrintf("[Tester]: Get Append round 3 OK, config 5 \n")

	cfg.StartServer(0, 0)
	cfg.StartServer(1, 0)
	cfg.StartServer(2, 0)
	DPrintf("[Tester]: restart server 0 in all group. \n")
	...
	DPrintf("[Tester]: Get Append round 4 OK, config 5 \n")
	...
	DPrintf("[Tester]: shutdown server 1 in all group. \n")

	cfg.join(0)
	cfg.leave(2)
	DPrintf("[Tester]: Join 0, leave 2, config[8] = 0+1. \n")
	...
	DPrintf("[Tester]: Get Append round 5 OK, config 8 \n")

	cfg.StartServer(0, 1)
	cfg.StartServer(1, 1)
	cfg.StartServer(2, 1)
	DPrintf("[Tester]: restart server 1 in all group. \n")

	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
	}
	DPrintf("[Tester]: Get round 6 OK, config 8 \n")
...
func TestUnreliable1(t *testing.T) {
	...
	cfg.join(0)
	cfg.leave(1)
	DPrintf("[Tester]: Join 0, Leave 1, config[6] = 0+2 \n")
